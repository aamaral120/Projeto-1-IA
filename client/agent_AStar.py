...
class Queue:
    ...
    def pop(self):
        self.queue_data.sort(key=Node.getEstimateCost)
...
class Node:
    def __init__(self, state, parent, action, path_cost, goal_position):
        self.state = state
        self.parent = parent
        self.action = action
        self.path_cost = path_cost
        self.heuristic = self.manhattan_distance(state, goal_position)
...
class Agent:
    ...
    def getNode(self, parent_node, action):
        ...
        return Node(state, parent_node, action, pathCost, self.goalNodePos)
    ...
    def think(self):
        ...
        # Add first node (root)
        root = Node(self.state, None, "", 0, self.goalNodePos)
